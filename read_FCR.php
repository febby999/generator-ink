...
foreach ($items as $key => $item) {
	...
	//$permalink = $http->validateUrl($permalink);
  		...
  		//clean redirect notice from google alert notice 
  		//$galertrss = 'https://www.google.com/url?rct=j&sa=t&url=http://time.com/money/4521124/recommendation-letters-scientists-gender-bias/&ct=ga&cd=CAIyHDE3MjRlMDA4YzI1MzQxZTk6Y28udWs6ZW46R0I&usg=AN6KhHsmc7XnPsfGcWj72kivPp_rJBfzpQ';
  		$permalink = str_replace('https://www.google.com/url?rct=j&sa=t&url=', '', $permalink);
  		$permalink = preg_replace('/&ct=ga&cd=[^&]*/', '', $permalink);
  		$permalink = preg_replace('/&usg=[^&]*/', '', $permalink);

  		$permalink = str_replace('https://www.google.com/url?sa=t&fd=R', '', $permalink);
  		$permalink = preg_replace('/&ct2=[^&]*/', '', $permalink);
  		$permalink = preg_replace('/&clid=[^&]*/', '', $permalink);
  		$permalink = preg_replace('/&cid=[^&]*/', '', $permalink);
  		$permalink = preg_replace('/&ei=[^&]*/', '', $permalink);
  		$permalink = str_replace('&url=', '', $permalink);
  		$permalink = str_replace('http://news.google.com/news/url?sa=t&fd=R', '', $permalink);	
