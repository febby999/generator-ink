...
////////////////////////////////
	...
	$getkeyword = str_replace(" ", "+", $_GET['keyword']);
	$end = "&cc=" .$lang ."&format=RSS";
	$googleend = "?&ned=" .$lang ."&hl=" .$lang;
if ($lang == "cn"){$url = "https://news.google.com/news/rss/search/section/q/" .urlencode($getkeyword) .$googleend;}
	else {$url = "http://www.bing.com/news/search?q=" .$getkeyword .$end;}
...
if (isset($_GET['key']) && ($key_index = array_search($_GET['key'], $options->api_keys)) !== false) {
	...
	$redirect = 'http://'.htmlspecialchars($host.$path).'/read.php?keyword='.urlencode($_GET['keyword']).$end;
